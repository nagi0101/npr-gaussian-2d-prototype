...
class UIController {
    constructor(client, canvasController) {
        ...
        this.showGaussianOrigins = document.getElementById(
            "showGaussianOrigins"
        );
        ...
        this.basisVectorLengthValue = document.getElementById(
            "basisVectorLengthValue"
        );
    ...
    updateBrushList(brushes) {
        ...
        if (brushes.length === 0) {
            this.brushLibraryList.innerHTML =
                '<div class="brush-library-item">No brushes available</div>';
            return;
        }
    ...
    selectBrush(brushId) {
        ...
        const items = this.brushLibraryList.querySelectorAll(
            ".brush-library-item"
        );
        items.forEach((item) => {
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    // Initialize UI controller (BEFORE callbacks that reference it)
    console.log("[App] Step 3: Initializing UI controller");
    uiController = new UIController(client, null);

    // Setup client callbacks
    console.log("[App] Step 4: Setting up WebSocket callbacks");
    client.onConnected = (data) => {
        ...
        client.onBrushCreated = (data) => {
            ...
            notification.textContent = `✓ Brush created: ${
                data.pattern || "custom"
            } (${data.num_gaussians || 0} Gaussians)`;
    ...
    console.log("[App] Step 7: Initializing brush preview");
    const brushPreview = new BrushPreview();

    // Initialize brush upload handler with preview
    console.log("[App] Step 8: Initializing brush upload handler");
    const uploadHandler = new BrushUploadHandler(client, brushPreview);
